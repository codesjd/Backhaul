...
* **Server Configuration**
   ...
   Create a configuration file named `config.toml`:
    ...
    mux_stripe = 1                 # wsmux/wssmux (EXPERIMENTAL): split a single flow's data across this many pool connections instead of pinning it to one, so one flow isn't capped by a single connection's congestion window/RTT. Must match on both server and client. 1 disables it (default, recommended). Fixed to no longer deadlock, and a lost/short stream now surfaces as a connection reset rather than silent corruption - but a residual teardown race can still reset the tail of a stream intermittently under load, so keep this at 1 for production and only raise it when experimenting with a single loss-limited flow. No leg-failure recovery: a pool connection dropping mid-transfer ends the flow. (optional, default: 1)
...
* **If one core is pinned while the other is idle** (a single big flow bottlenecked on one connection's copy/decrypt), the mux transports' `mux_stripe = N` splits that one flow across N pooled connections so it can use more than one core - or, more commonly on an intercontinental link, across N congestion windows so per-flow packet loss doesn't cap it. It's **experimental** (see the option docs): the fatal deadlock is fixed and failures are now loud rather than silently corrupting, but a residual teardown race can intermittently reset a stream's tail, so keep it at 1 for production. If *both* cores are already saturated (aggregate-bound), striping won't help anyway - you're out of CPU, so reduce per-byte cost with the points above instead.
