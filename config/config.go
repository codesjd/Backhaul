...
type ClientConfig struct {
	...
	SO_SNDBUF            int           `toml:"so_sndbuf"`
	Path                 string        `toml:"path"`
	TLSVerify            *bool         `toml:"tls_verify"` // wss/wssmux: verify the server's TLS certificate. On by default; disable for self-signed setups, but beware this allows an on-path party to MITM the token-bearing handshake.
}
